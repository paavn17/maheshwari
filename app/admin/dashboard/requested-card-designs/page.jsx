...
export default function InstituteCardDesignsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="p-6">
        <h1 className="text-3xl font-bold mb-6 text-orange-700">Institute Card Designs</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
            {cards.map(card => (
              <div
                key={card.id}
                className="bg-white rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition-shadow duration-300 p-4 flex flex-col space-y-3"
              >
                <h2 className="text-xl font-semibold text-orange-600 mb-2">{card.name}</h2>
                  ...
                  {card.front_img && (
                    <div className="overflow-hidden rounded-lg border border-gray-300 shadow-sm">
                      <img
                        src={card.front_img}
                        alt="Front"
                        className="w-full h-[400px] object-cover hover:scale-105 transition-transform duration-200"
                      />
                    </div>
                  )}
                  {card.back_img && (
